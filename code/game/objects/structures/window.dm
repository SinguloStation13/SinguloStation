/obj/structure/window
	...
	var/decon_speed = 12 //adjust to your liking lower = faster
	var/wtype = "glass"
	var/fulltile = FALSE
...
/obj/structure/window/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	if(the_rcd.mode == RCD_DECONSTRUCT)
		return list("delay" = decon_speeddecon_speed, "cost" = 5)
	return FALSE
...
/obj/structure/window/reinforced/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	if(the_rcd.mode == RCD_DECONSTRUCT)
		return list("delay" = decon_speed, "cost" = 15)
	return FALSE
...
/obj/structure/window/reinforced/wirecutter_act(mob/living/user, obj/item/tool)
	if((obj_flags & NO_DECONSTRUCTION) ||(state != RWINDOW_POPPED))
		return FALSE
...
/obj/structure/window/reinforced/wrench_act(mob/living/user, obj/item/tool)
	if(obj_flags & NO_DECONSTRUCTION)
		return FALSE
	switch(state)
		if(RWINDOW_BARS_CUT)
		...
		if((WINDOW_OUT_OF_FRAME) && (!anchored))
			to_chat(user, span_notice("You begin to disassemble [src]..."))
			if(!tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				return ITEM_INTERACT_SUCCESS
			var/obj/item/stack/sheet/G = new glass_type(user.loc, glass_amount)
...
/obj/structure/window/fulltile/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	if(the_rcd.mode == RCD_DECONSTRUCT)
		return list("delay" = decon_speed, "cost" = 10)
	return FALSE
...
/obj/structure/window/reinforced/fulltile/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	if(the_rcd.mode == RCD_DECONSTRUCT)
		return list("mode" = RCD_DECONSTRUCT, "delay" = decon_speed, "cost" = 20)
	return FALSE
