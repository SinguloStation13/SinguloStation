...
/obj/machinery/computer/cryopod/ui_data(mob/user)
	...
	for (var/mob/living/L in frozen_crew)
		...
		for(var/datum/record/G as anything in GLOB.manifest.general) // Get the rank the dude
			if(G.name == L.real_name)
				data_set["rank"] = G.rank
...
/obj/machinery/cryopod/proc/store_occupant()
	...
	for(var/datum/record/G as anything in GLOB.manifest.general)
